...
# @Last Modified time: 2023-07-27 23:08:34
...
if __name__ == '__main__':
    ...
    for i in sets:
        ...
        os.makedirs(ROOT_DIR+i+"/"+LABEL_SAVE_FOLDER, exist_ok=True)
    ...
    for i in tqdm(range(len(valid_images))):
        ...
        # 生成目标文件夹中图片的新路径
        image_destination_folder = os.path.join(destination, IMAGE_FOLDER)
        # 移动图片到目标文件夹
        shutil.copy(image_path, image_destination_folder)
        # 生成目标文件夹中 label 的新路径
        label_destination_folder = os.path.join(destination, LABEL_SAVE_FOLDER)
        # 移动 label 到目标文件夹
        shutil.copy(label_path, label_destination_folder)
